/**
 * Kept identical to cre/placement-verifier/checks.ts, on purpose.
 *
 * The confidential workflow can't import from outside its own bundled
 * folder, so this is a deliberate copy rather than a shared import — used
 * only by the manual-verdict bypass (src/lib/verification/mock-verify.ts)
 * while Chainlink CRE is unreachable. If you touch the logic here, touch it
 * there too; the CRE test suite (cre/placement-verifier/checks.test.ts)
 * covers both by construction since the code is identical.
 */
